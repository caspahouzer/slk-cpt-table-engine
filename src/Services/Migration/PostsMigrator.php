...
final class PostsMigrator
{
     ...
     *
     * @param string $post_type The post type to migrate.
     * @return array|WP_Error An array containing the ID map, or a WP_Error on failure.
     */
    public static function migrate_to_wp_posts(string $post_type)
    {
        ...
        $conflicting_ids = self::check_id_conflicts($custom_table, $post_type);
        $id_map = [];
        ...
        while ($offset < $total) {
            ...
            foreach ($posts as $post) {
                ...
                if (in_array($original_id, $conflicting_ids)) {
                    // Handle conflict: insert post to get a new ID
                    unset($post['ID']);
                    $new_id = wp_insert_post($post, true);

                    if (is_wp_error($new_id)) {
                        Logger::error("Failed to re-insert conflicting post '{$post['post_title']}'. Error: " . $new_id->get_error_message());
                        continue;
                    }
                    $id_map[$original_id] = $new_id;
                    Logger::info("Resolved conflict for post {$original_id}. New ID is {$new_id}.");
                } else {
                    // No conflict, insert with original ID
                    self::insert_post_with_id($post);
                }
                $migrated++;
            }
    ...
    /**
     * Insert a single post with a specific ID.
     *
     * @param array $post
     * @return int|false
     */
    private static function insert_post_with_id(array $post)
    {
        global $wpdb;
        return $wpdb->insert($wpdb->posts, $post);
    }
     ...
     * @return array An array of conflicting post IDs.
     */
    private static function check_id_conflicts(string $custom_table, string $post_type): array
    {
        global $wpdb;
        ...
        if (!empty($conflicting_ids)) {
            Logger::warning(
                sprintf(
                    'ID conflicts detected during reverse migration for post type \'%s\': %d IDs already exist in wp_posts. Conflicting IDs: %s',
                    $post_type,
                    count($conflicting_ids),
                    implode(', ', $conflicting_ids)
                )
            );
            return array_map('intval', $conflicting_ids);
        }
